---
title: this
date: 2017-04-16 21:09:11
tags:
categories: javascript
---

函数中的 this 到底是指向哪里，bind,call,apply 是如何改变 this 指向的。总之，函数被调用时到底是如何确定 this 的。
...
JavaScript 的类型分为语言类型和规范类型。
JavaScript 语言类型是开发者直接使用 JavaScript 可以操作的， 包括 Undefined, Null, Boolean, String, Number, 和 Object。
规范类型相当于 meta-values，是用来用算法描述 JavaScript 语言结构和 JavaScript 语言类型的，包括：Reference, List, Completion, Property Descriptor, Property Identifier, Lexical Environment, 和 Environment Record。。
规范类型，也就是 “只存在于规范里的抽象类型”。它们的作用是用来描述语言底层行为逻辑，是为了更好地描述语言的底层行为逻辑才存在的，但并不存在于实际的 js 代码中。

Reference 类型是用来解释诸如 delete、typeof 以及赋值等操作行为的。

再看接下来的这段具体介绍 Reference 的内容：

A Reference is a resolved name binding，A base value of undefined indicates that the reference could not be resolved to a binding.

Reference 包含三个部分：

- base value
  base value 的值只可能是 undefined, an Object, a Boolean, a String, a Number, or an environment record 其中的一种。
- referenced name
  属性的名称，是一个字符串
...
除此之外，规范中讲了一个用于从 Reference 类型获取对应值的方法： GetValue。
...
此外，又如何确定调用函数的对象是谁呢？，尽管 foo() 和 (foo.bar = foo.bar)() 最后结果都指向了 undefined，但是两者从规范的角度上却有着本质的区别。
